<?php
namespace App\Empower\RestClient;
/**
 * PHP REST Client
...
class RestClient implements \Iterator, \ArrayAccess {
    ...
    public function post($url, $parameters=[], $headers=[], $send_type=null){
        return $this->execute($url, 'POST', $parameters, $headers, $send_type);
    }
    
    public function put($url, $parameters=[], $headers=[], $send_type=null){
        return $this->execute($url, 'PUT', $parameters, $headers, $send_type);
    }
    
    public function patch($url, $parameters=[], $headers=[], $send_type=null){
        return $this->execute($url, 'PATCH', $parameters, $headers, $send_type);
    }
    
    public function delete($url, $parameters=[], $headers=[], $send_type=null){
        return $this->execute($url, 'DELETE', $parameters, $headers, $send_type);
    }
    
    public function head($url, $parameters=[], $headers=[], $send_type=null){
        return $this->execute($url, 'HEAD', $parameters, $headers, $send_type);
    }
    
    public function execute($url, $method='GET', $parameters=[], $headers=[], $send_type=null){
        $client = clone $this;
        $client->url = $url;
        ...
        if(count($client->options['headers']) || count($headers)){
			...
			if( isset($send_type) && ($send_type == "json" || $send_type == "json_body") ){
				$curlopt[CURLOPT_HTTPHEADER] = ['Content-Type:application/json'];
            }

            if( isset($send_type) && $send_type == "xml"){
				$curlopt[CURLOPT_HTTPHEADER] = ['Content-Type:text/xml'];
            }
        ...
        if(is_array($parameters)){
            ...
            if( isset($send_type) && $send_type == "xml"){
                ...
                //echo $parameters_string;die;
            }
			
			// generate XML to send       
            if( isset($send_type) && $send_type == "json_body"){
		...
		curl_setopt($client->handle, CURLOPT_FOLLOWLOCATION, TRUE);
        curl_setopt($client->handle, CURLOPT_SSL_VERIFYPEER, false);
		curl_setopt($client->handle, CURLOPT_TIMEOUT, 5000);
		curl_setopt($client->handle, CURLOPT_SSL_VERIFYHOST, false); // Ignore cert errors?
		//curl_setopt($client->handle, CURLOPT_PROXY, false);  
		// dd(curl_getinfo($client->handle));
